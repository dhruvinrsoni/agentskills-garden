...
name: cleanup
description: >
  Remove noise, enforce formatting, and safely rename identifiers. A
  mixed-mode skill: cosmetic fixes run in Eco, renaming runs in Power.
license: Apache-2.0
compatibility: Designed for Claude Code and compatible AI agent environments
domain: engineering
status: published
tags: [category, build, refactoring, reversible]
keywords: [dead code removal, unused imports, formatting, prettier, eslint, black, ruff, rename, intent markers, whitespace, linter, identifier]
metadata:
  version: 1.0.0
