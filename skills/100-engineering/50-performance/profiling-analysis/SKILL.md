...
description: >
  Analyze application performance using profiling tools, traces, and
  logs to identify bottlenecks and hotspots.
license: Apache-2.0
compatibility: Designed for Claude Code and compatible AI agent environments
domain: engineering
status: published
tags: [category, operate, performance, advisory]
keywords: [flame graph, cpu profiling, memory leak, heap snapshot, pprof, py-spy, opentelemetry, distributed tracing, hotspot, p99, load testing]
metadata:
  version: 1.0.0
