...
### 0. Layered Index Lookup 🌿 (Eco Mode) — the entry point at scale

**Goal:** Find the right skill among thousands without loading them all. This is
the scale-critical path; run it before the matching steps below. Walk:
**map → domain index → category index → skill.**

**Steps:**

1. Load the **map** — `skills/_index/MAP.md`. It lists *domains only* (foundation
   / engineering / writing / …), each with a count and a link to its index.
2. Classify the user's intent to a **domain**.
3. Open that domain's **index** — `skills/_index/<domain>/INDEX.md`:
   - **Flat domain** (e.g. `foundation`) → the INDEX lists skills directly.
     Skip to step 5.
   - **Otherwise** → the INDEX lists the domain's **categories**. Classify intent
     to a **category** (reuse the Intent Classification table below).
4. Load the **category index** — only the matched table
   `skills/_index/<domain>/<category>.md`: `name | what it does | keywords | path`.
5. Run the Graduated Matching waterfall (Micro-Skill 2) against THAT table only,
   matching the query against name + description + **keywords**.
6. Read the full **`SKILL.md`** for the 1–3 winning skills via the `path` column.
7. If the domain/category is genuinely ambiguous, load the two or three most
   likely tables (still tiny) rather than scanning everything.

**Why:** cost per lookup = MAP.md (domains only, fixed) + one domain index +
one category table (bounded by skills-per-category) + 1–3 skills. None grow with
...
| 4 | **TAG** | 0.50 | Query matches a tag or keyword in the Tier-1 index (`"resilience"` → `resilience-patterns`) |
...
| `index`        | `string` | no       | Path to skills/_index/ (auto-detected); Tier-0 map is `skills/_index/MAP.md` |
